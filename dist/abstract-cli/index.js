...
function resolveBinDir() {
  return path.resolve(__dirname, "bin").replace(/[\\\/]app.asar[\\\/]/, `${path.sep}app.asar.unpacked${path.sep}`);
}
...
function resolveCLIBinary() {
  ...
  if (process.platform === "darwin" || process.platform === "linux" || process.platform === "win32") {
    ...
    return binPath; // Fix issue with fs.accessSync
    // try {
  ...
  throw new Error(`${BINARY_NAME} not supported on platform: ${process.platform}`);
}
