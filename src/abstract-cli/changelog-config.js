...
module.exports = {
  ...
  recommendedBumpOpts: {
    ...
    whatBump: (commits, _) => {
      ...
      commits.forEach(commit => {
        ...
        // If we have a note, it's a breaking change (0)
        if (commit.notes.length > 0) {
          breakings += commit.notes.length
          level = 0
        } else if (commit.type === `feat`) {
          features += 1
          // If we have a feat, it's a minor change (1) but we only
          ...
          refactors += 1
          // Because most of our codebase is basically the CLI, and because
      ...
      return {
        level: level,
        reason: breakings === 1
          ? `There is ${breakings} BREAKING CHANGE, ${features} feature(s), and ${refactors} refactor(s)`
          : `There are ${breakings} BREAKING CHANGES, ${features} feature(s), and ${refactors} refactor(s)`
      }
    },
  },
  ...
  writerOpts: {
      ...
      transform: (commit, context) => {
        ...
        // The way it works is that, if we have a note it has to be
        ...
        if (typeof commit.subject === 'string') {
          ...
          if (url) {
            url = `${url}/issues/`
            // because the CLI is public but the repo is private,
            // we use the SDK URL.
            ...
            url = url.replace("abstract-sdk", "projects")
